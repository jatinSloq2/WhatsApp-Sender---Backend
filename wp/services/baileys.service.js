...
const MIN_RECONNECT_DELAY = 2000;  // 2 sec
const MAX_RECONNECT_DELAY = 60000; // 1 min cap
//const KEEP_ALIVE_INTERVAL = 30_000; // ms
const KEEP_ALIVE_INTERVAL = 90_000; // ms
//const PONG_TIMEOUT = 60_000;       // ms - force reconnect if no pong in this period
const PONG_TIMEOUT = 600_000;       // ms - force reconnect if no pong in this period
...
// ======= Retry logic =======
function getNextRetryDelay(sessionId) {
  const attempts = retries.get(sessionId) ?? 0;
  // Exponential backoff: 2s, 4s, 8s, 16s, 32s, 60s...
  const delay = Math.min(MIN_RECONNECT_DELAY * Math.pow(2, attempts), MAX_RECONNECT_DELAY);
  return delay;
}
...
function shouldReconnect(sessionId) {
  ...
  log(sessionId, `Reconnect attempt ${attempts + 1} (next delay: ${getNextRetryDelay(sessionId)}ms)`);
  return true;
}
    ...
    try {
      ...
      if (connection === "close") {
        ...
        if (shouldReconnect(sessionId)) {
          const delay = getNextRetryDelay(sessionId);
          log(sessionId, `Reconnecting in ${delay} ms...`);
          setTimeout(() => {
        ...
        // Only explicitly delete if shouldReconnect returns false (which it won't anymore for normal disconnects)
        log(sessionId, "Reconnect disabled by logic. Stopping retry loop.");
        socketActive.set(sessionId, false);
